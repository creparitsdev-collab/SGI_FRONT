...
import { getUnitsOfMeasurement } from "../service/product"
...
import { UnitsOfMeasurementDrawer } from "../components/unitsOfMeasurement/UnitsOfMeasurementDrawer"
import { UnitsOfMeasurementDeleteModal } from "../components/unitsOfMeasurement/UnitsOfMeasurementDeleteModal"
...
export const UnitsOfMeasurement = () => {
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredUnits = filteredUnits.filter((unit) =>
                ...
                unit.description?.toLowerCase().includes(searchValue.toLowerCase())
            )
        }
    ...
    const paginatedSortedItems = React.useMemo(() => {
        return sortedItems.map((unit, idx) => ({
            ...unit,
            pageIndex: idx,
        }))
    }, [sortedItems])
        ...
        return (
            <div className="flex justify-between gap-4 items-center px-1 n2">
                <div className="flex flex-col n7">
                    <p className="text-lg font-bold">Unidades de medida</p>
    ...
    return (
        <>
                                                            ...
                                                            <DropdownMenu aria-label="Acciones" variant="light" itemClasses={{base:"mt-1 mb-2"}}>
                                                                <DropdownSection title="Acciones" classNames={{ heading: "text-background-500 font-normal"}}>
                                                                    ...
                                                                    <DropdownItem 
                                                                        ...
                                                                        key="handleUpdateUnit"
                                                                        startContent={<EditFilled className="size-5"/>}
                                                                        onPress={() => handleUpdateUnit(item)}
                                                                    >
                                                                        Actualizar unidad
                                                                    </DropdownItem>}
            ...
            <UnitsOfMeasurementDeleteModal isOpen={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen} data={selectedUnit} onRefresh={triggerRefresh}/>
            <UnitsOfMeasurementDrawer isOpen={isDrawerOpen} onOpenChange={setIsDrawerOpen} data={selectedUnit} action={action} onRefresh={triggerRefresh}/>
        </>
    )
}
