...
import { getProductStatuses, createProductStatus, updateProductStatus, deleteProductStatus } from "../service/product"
...
import { ProductStatusesDrawer } from "../components/productStatuses/ProductStatusesDrawer"
import { ProductStatusesDeleteModal } from "../components/productStatuses/ProductStatusesDeleteModal"
...
export const ProductStatuses = () => {
    ...
    const filteredItems = React.useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredStatuses = filteredStatuses.filter((status) =>
                ...
                status.description?.toLowerCase().includes(searchValue.toLowerCase())
            )
        }
    ...
    const paginatedSortedItems = React.useMemo(() => {
        return sortedItems.map((status, idx) => ({
            ...status,
            pageIndex: idx,
        }))
    }, [sortedItems])
        ...
        return (
            <div className="flex justify-between gap-4 items-center px-1 n2">
                <div className="flex flex-col n7">
                    <p className="text-lg font-bold">Estados de Productos</p>
                ...
                <div className="flex gap-2 sm:gap-4">
                    ...
                    <PrimaryButton
                        ...
                        onPress={() => {handleCreateStatus(); setIsDrawerOpen(true)}}
                    />}
                </div>
            </div>
        )
        ...
        <>
            {isLoading ? (
                ...
                <Table
                    ...
                    radius="none"
                    shadow="none"
                    aria-label="Tabla de estados de productos"
                        ...
                        {(item) => (
                            <TableRow aria-label={item.n} key={item.n} className="hover:-translate-y-1 transition-all duration-250 ease-in-out">
                                <TableCell className="px-0 py-1">
                                    ...
                                    >
                                        <Card shadow="none" radius="sm" isPressable onPress={() => {handleReadStatus(item); setIsDrawerOpen(true)}} className="w-full transition-colors !duration-1000 ease-in-out bg-transparent dark:bg-background-100 shadow-small">
                                                            ...
                                                            <DropdownMenu aria-label="Acciones" variant="light" itemClasses={{base:"mt-1 mb-2"}}>
                                                                <DropdownSection title="Acciones" classNames={{ heading: "text-background-500 font-normal"}}>
                                                                    ...
                                                                    <DropdownItem 
                                                                        ...
                                                                        onPress={() => {handleUpdateStatus(item); setIsDrawerOpen(true)}}
                                                                    >
                                                                        Actualizar estado
                                                                    </DropdownItem>}
                                                                    ...
                                                                    <DropdownItem 
                                                                        ...
                                                                        key="handleReadStatus"
                                                                        startContent={<ScriptFilled className="size-5"/>}
                                                                        onPress={() => {handleReadStatus(item); setIsDrawerOpen(true)}}
                                                                    >
                                                                        Ver más detalles
                                                                    </DropdownItem>
                                                                    ...
                                                                    <DropdownItem 
                                                                        ...
                                                                        key="handleDeleteStatus"
                                                                        startContent={<DeleteFilled className="size-5"/>}
                                                                        onPress={() => handleDeleteStatus(item)}
                                                                    >
                                                                        Eliminar
                                                                    </DropdownItem>}
                                                                </DropdownSection>
            ...
            <ProductStatusesDeleteModal isOpen={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen} data={selectedStatus} onRefresh={triggerRefresh}/>
            <ProductStatusesDrawer isOpen={isDrawerOpen} onOpenChange={setIsDrawerOpen} data={selectedStatus} action={action} onRefresh={triggerRefresh}/>
        </>
    )
}
