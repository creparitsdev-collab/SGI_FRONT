...
import { createStockCatalogue, updateStockCatalogue } from "../../service/product"

export const StockCataloguesModal = ({isOpen, onOpenChange, data, initialData, action, onRefresh, closeDrawer}) => {
    const targetRef = useRef(null)
    const {moveProps} = useDraggable({targetRef, isDisabled: !isOpen})

    const [showBefore, setShowBefore] = useState(false)
    const description = action === "create"
    ? "Una vez registrado, el catálogo estará disponible para ser utilizado en los productos."
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = action === "create"
                ...
                : await updateStockCatalogue(catalogueData)

            const success = response.type === "SUCCESS"
            
            addToast({
                title: success
                    ? `Se ${verb} el catálogo ${data.name}`
                    : `No se ${verb} el catálogo ${data.name}`,
                description: success ? "El catálogo está disponible para su uso." : "Ocurrió un error al procesar la solicitud.",
            ...
            addToast({
                title: `No se ${verb} el catálogo ${data.name}`,
    ...
    const catalogueDetails = (catalogue) => {
        return (
                ...
                <CardBody className="pl-4">
                    ...
                    <div className="w-full flex flex-col gap-1">
                        <div className="w-full flex justify-between">
                            <p className="font-semibold break-all line-clamp-2 pr-4">{catalogue.name}</p>
                            {action !== "create" && (
                                <Tooltip
                        ...
                        {catalogue.description && <p className="text-sm line-clamp-3 break-all"><span className="font-medium">Descripción: </span>{catalogue.description}</p>}
                    </div>
                </CardBody>
            </Card>
        );
                ...
                <ModalContent className="bg-background">
                    {(onClose) => (
                        ...
                        <ModalHeader {...moveProps} className="flex flex-col gap-2 pb-4 pt-4">
                            ...
                            <p className="text-lg font-bold text-center">¿Desea {action === "create" ? "registrar" : "actualizar"} el siguiente catálogo?</p>
                        </ModalHeader>
                        <ModalBody className="py-0 gap-0">
                            <p className="text-sm font-normal pb-4 text-center">{description}</p>

                            {action === "create" ? 
                                catalogueDetails(data)
                            :
                                !showBefore ? catalogueDetails(data) : catalogueDetails(initialData)
                            }
                        </ModalBody>
