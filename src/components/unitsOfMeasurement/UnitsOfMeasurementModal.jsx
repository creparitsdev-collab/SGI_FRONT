...
import { createUnitOfMeasurement, updateUnitOfMeasurement } from "../../service/product"

export const UnitsOfMeasurementModal = ({isOpen, onOpenChange, data, initialData, action, onRefresh, closeDrawer}) => {
    const targetRef = useRef(null)
    const {moveProps} = useDraggable({targetRef, isDisabled: !isOpen})

    const [showBefore, setShowBefore] = useState(false)
    const description = action === "create"
    ? "Una vez registrada, la unidad estará disponible para ser utilizada en los productos."
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast({
                title: success
                    ? `Se ${verb} la unidad ${data.name}`
                    : `No se ${verb} la unidad ${data.name}`,
                description: success ? "La unidad está disponible para su uso." : "Ocurrió un error al procesar la solicitud.",
            ...
            addToast({
                title: `No se ${verb} la unidad ${data.name}`,
                ...
                <CardBody className="pl-4">
                    ...
                    <div className="w-full flex flex-col gap-1">
                        <div className="w-full flex justify-between">
                            <p className="font-semibold break-all line-clamp-2 pr-4">{unit.name}</p>
                            {action !== "create" && (
                                <Tooltip
                        ...
                        {unit.code && <p className="text-sm break-all"><span className="font-medium">Código: </span>{unit.code}</p>}
                        {unit.description && <p className="text-sm line-clamp-3 break-all"><span className="font-medium">Descripción: </span>{unit.description}</p>}
                    </div>
                </CardBody>
            </Card>
        )
                ...
                <ModalContent className="bg-background">
                    {(onClose) => (
                        ...
                        <ModalHeader {...moveProps} className="flex flex-col gap-2 pb-4 pt-4">
                            ...
                            <p className="text-lg font-bold text-center">¿Desea {action === "create" ? "registrar" : "actualizar"} la siguiente unidad?</p>
                        </ModalHeader>
                        <ModalBody className="py-0 gap-0">
                            ...
                            {action === "create" ? 
                                unitDetails(data)
                            :
                                !showBefore ? unitDetails(data) : unitDetails(initialData)
                            }
                        </ModalBody>
