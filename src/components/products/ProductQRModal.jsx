...
export const ProductQRDrawer = ({ isOpen, onOpenChange, product }) => {
  // Eliminamos useDraggable y useRef ya que no se usan en el Drawer
  ...
  // --- LÓGICA DE FORMATEO (Mantenida intacta) ---
  ...
  return (
    <Drawer
      ...
      // Estilos idénticos al ProductsDrawer de ejemplo
    ...
    >
      <DrawerContent className="bg-background">
        {(onClose) => (
          <>
            {/* Header con el mismo estilo y layout */}
            ...
            {/* Body con las mismas clases de scroll personalizadas y layout flex-col justify-between */}
            <DrawerBody className="h-full flex flex-col justify-between overflow-x-hidden [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary">
              {/* Contenedor del contenido principal (QR y Datos) */}
              ...
              {/* Botón de descarga al final del DrawerBody, tal como en el ejemplo */}
