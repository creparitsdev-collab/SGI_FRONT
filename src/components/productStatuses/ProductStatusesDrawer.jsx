...
import { ArrowHookUpRightFilled, CheckmarkFilled, CircleFilled, DismissCircleFilled, DismissFilled, TextAsteriskFilled } from "@fluentui/react-icons"
import { useEffect, useState } from "react"
import { required } from "../../js/validators"
import { ProductStatusesModal } from "./ProductStatusesModal"
import { formatDateLiteral } from "../../js/utils"

export const ProductStatusesDrawer = ({isOpen, onOpenChange, data, action, onRefresh}) => {
    ...
    const resetForm = () => {
        setStatus({ id:"", name:"", description:"" })
        setStatusErrors({ name:[], description:[] })
    }
    ...
    switch (action) {
        case "create":
            ...
            description = "Ingrese la información solicitada para poder registrar un nuevo estado de producto."
            break
        case "update":
            title = "Actualizar estado"
            description = "Edite la información necesaria y guarde los cambios para actualizar el estado."
            break
        default:
            title = "Detalles del estado"
            description = "Revise la información completa del estado. Esta vista es solo de lectura."
            break
    }
    ...
    return (
        <>
                ...
                <DrawerContent className="bg-background">
                    {(onClose) => (
                        ...
                        <DrawerBody className="h-full flex flex-col justify-between [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary">
                            <Form onSubmit={onSubmit} id="status-form" className={action === 'create' || action === 'update' ? "gap-6 flex flex-col" : "gap-6 flex flex-col pb-8"}>
                                {action !== "create" && data && (
                                ...
                                <Input
                                    label={
                                        <div className="flex justify-between">
                                            ...
                                            <p className="!text-background-500 text-xs font-normal">{status.name.length + " / 255"}</p>
                                        </div>
                                    }
                                    ...
                                    placeholder={action === "create" ? "Ingrese el nombre del estado" : data?.name}
                                    value={status.name}
                                    onValueChange={(value) => handleInputChange('name', value)}
                                    isInvalid={statusErrors.name.length > 0}
                                    endContent={statusErrors.name.length === 0 && status.name ? <CheckmarkFilled className='size-4 text-background-500 group-data-[focus=true]:text-primary' /> : statusErrors.name.length > 0 ? <DismissFilled className='size-4 text-danger' /> : null }
                                    errorMessage={() => (
                                        <div className="flex text-danger">
                                            <ul>
                                                {statusErrors.name.map((error, i) => (
                                                    <li key={i}>{error}</li>
                                                ))}
                                            </ul>
                                        </div>
                                ...
                                <Textarea
                                    label={
                                        <div className="flex justify-between">
                                            ...
                                            <p className="!text-background-500 text-xs font-normal">{status.description.length + " / 500"}</p>
                                        </div>
                                    }
                                    classNames={{ label: "w-full font-medium !text-current transition-colors !duration-1000 ease-in-out", input: "transition-colors !duration-1000 ease-in-out group-data-[invalid=true]:!text-current font-medium !placeholder-background-500 placeholder:!font-normal",  mainWrapper: "group-data-[invalid=true]:animate-shake", inputWrapper: "transition-colors !duration-1000 ease-in-out caret-primary group-data-[invalid=true]:caret-danger bg-background-100 group-data-[hover=true]:border-background-200 group-data-[focus=true]:!border-primary group-data-[invalid=true]:!border-danger border-transparent text-current" }}
                                    className="w-full"
                                    ...
                                    minRows={3}
                                    isReadOnly={action !== 'create' && action !== 'update'}
                                    placeholder={action === "create" ? "Ingrese la descripción del estado (Opcional)" : data?.description || "Sin descripción"}
                            ...
                            {(action === 'create' || action === 'update') && (
                                <div className="w-full flex justify-end py-8 sm:gap-4 gap-2">
                                    <Button
                                        ...
                                        isLoading={isLoading}
                                        isDisabled={status.name === "" || statusErrors.name.length > 0}
                                    >
                                        Siguiente
                                    </Button>
                                </div>
            ...
            <ProductStatusesModal isOpen={isModalOpen} onOpenChange={onModalOpenChange} data={status} initialData={data} action={action} onRefresh={onRefresh} closeDrawer={() => {onOpenChange(false); resetForm()}}/>
        </>
    )
}
