...
import { createProductStatus, updateProductStatus } from "../../service/product"

export const ProductStatusesModal = ({isOpen, onOpenChange, data, initialData, action, onRefresh, closeDrawer}) => {
    const targetRef = useRef(null)
    const {moveProps} = useDraggable({targetRef, isDisabled: !isOpen})

    const [showBefore, setShowBefore] = useState(false)
    const description = action === "create"
    ? "Una vez registrado, el estado estará disponible para ser utilizado en los productos."
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = action === "create"
                ...
                : await updateProductStatus(statusData)

            const success = response.type === "SUCCESS"
            
            addToast({
                title: success
                    ? `Se ${verb} el estado ${data.name}`
                    : `No se ${verb} el estado ${data.name}`,
                description: success ? "El estado está disponible para su uso." : "Ocurrió un error al procesar la solicitud.",
            ...
            addToast({
                title: `No se ${verb} el estado ${data.name}`,
                ...
                <CardBody className="pl-4">
                    ...
                    <div className="w-full flex flex-col gap-1">
                        <div className="w-full flex justify-between">
                            <p className="font-semibold break-all line-clamp-2 pr-4">{status.name}</p>
                            {action !== "create" && (
                                <Tooltip
                        ...
                        {status.description && <p className="text-sm line-clamp-3 break-all"><span className="font-medium">Descripción: </span>{status.description}</p>}
                    </div>
                </CardBody>
            </Card>
        );
                ...
                <ModalContent className="bg-background">
                    {(onClose) => (
                        ...
                        <ModalHeader {...moveProps} className="flex flex-col gap-2 pb-4 pt-4">
                            ...
                            <p className="text-lg font-bold text-center">¿Desea {action === "create" ? "registrar" : "actualizar"} el siguiente estado?</p>
                        </ModalHeader>
                        <ModalBody className="py-0 gap-0">
                            ...
                            {action === "create" ? 
                                statusDetails(data)
                            :
                                !showBefore ? statusDetails(data) : statusDetails(initialData)
                            }
                        </ModalBody>
